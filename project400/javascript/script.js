...
class Node {
  ...
  onClick() {
    if (
      Node.linking &&
      this.node != Node.linkingNode.node &&
      !(this.id in Node.linkingNode.links)
    ) {
      ...
      ).appendTo(`#${this.links_id}`);
      $(
        this.linkRow(
          Node.linkingNode.links_id,
          start_node,
          Node.linkingNode.weight_id,
          Node.linkingNode.toggleLink_id,
          Node.linkingNode.deleteLink_id
        )
      ).appendTo(`#${Node.linkingNode.links_id}`);
  ...
  mousemove(event) {
    if (this.isDown) {
      ...
      if (
        y > top + this.node.height() / 2 &&
        y < bottom - this.node.height() / 2 &&
        x > left + this.node.width() / 2 &&
        x < right - this.node.width() / 2
      ) {
  ...
  showPathTable() {
    ...
    let centered =
      this.node.position().left - (this.menu.width() - this.node.width()) / 2;
  ...
  toggleLink(event) {
    ...
    let target_id = id.substring(
      this.toggleLink_id.length + "_".length,
      id.length
    );
    if ($(event.target).html() == "-") {
      ...
      $(`#${Node.nodes[target_id].deleteLink_id + "_" + this.id}`).prop(
        "disabled",
        true
      );
      $(`#${this.weight_id + "_" + target_id}`).prop("disabled", true);
      $(`#${Node.nodes[target_id].weight_id + "_" + this.id}`).prop(
        "disabled",
        true
      );
    ...
    if ($(event.target).html() == "+") {
      ...
      for (let i = 0; i < Node.disabled.length; i++) {
        if (
          Node.disabled[i][0] == this.id &&
          Node.disabled[i][1] == target_id
        ) {
          removeValues.push(Node.disabled[i]);
          Node.links.push([this.id, target_id]);
        }
        if (
          Node.disabled[i][0] == target_id &&
          Node.disabled[i][1] == this.id
        ) {
          removeValues.push(Node.disabled[i]);
      ...
      //readd weight
      this.links[target_id] = parseInt(
        $(`#${this.weight_id + "_" + target_id}`).val()
      );
      Node.nodes[target_id].links[this.id] = parseInt(
        $(`#${Node.nodes[target_id].weight_id + "_" + this.id}`).val()
      );
      Node.SPF();
      for (const [target, node] of Object.entries(Node.nodes)) {
        node.updateTable();
  ...
  deleteLink(event) {
    let id = $(event.target).attr("id");
    let target_id = id.substring(
      this.deleteLink_id.length + "_".length,
      id.length
    );
  ...
  updateTable() {
    for (const [target, node] of Object.entries(Node.nodes)) {
      ...
      if (
        this.distances[target] != Number.MAX_VALUE &&
        this.distances[target] != Number.MAX_VALUE
      ) {
        $(
          this.pathRow(
            this.paths_id,
            target,
            this.distances[target],
            this.forwarding[target]
          )
        ).appendTo(`#${this.paths_id}`);
      }
    }
  }
  ...
  pathRow(paths_id, target, distance, forward) {
    return `<tr id='${
      paths_id + "_" + target
    }'><td>${target}</td><td>${distance}</td><td>${forward}</td></tr>`;
  }
  ...
  linkRow(link_id, target_id, weight_id, toggleLink_id, deleteLink_id) {
    return `<tr id=${link_id + "_" + target_id}>
                ...
                <td><input class='${weight_id}' id='${
      weight_id + "_" + target_id
    }' style="width:50px" value='1' min='1' type='number'/></td>
                <td><button class='${toggleLink_id}' id='${
      toggleLink_id + "_" + target_id
    }' style='width: 100%'>-</button></td>
                <td><button class='${deleteLink_id}' id='${
      deleteLink_id + "_" + target_id
    }' style='width: 100%'>x</button></td>
            </tr>`;
  }
  ...
  createMenu(top, left) {
    return `
        <div draggable='false' id='${
          this.menu_id
        }' style='top: ${top}%; left: ${left}%; display: none; position: absolute;'>
            <table>
                <thead>
                    <th>Node ${this.id}</th>
                </thead>
                <tbody>
                    <tr><td>
                        <table id='${"table_" + this.links_id}' colspan=2>
                            <thead>
                                <th colspan=4>Links: <button class='next' id='${
                                  this.next_id
                                }' >></button></th>
                            </thead>
                        ...
                        <table id='${
                          "table_" + this.paths_id
                        }' style="display: none;" colspan=3>
                            <thead>
                                <th colspan=3><button class='previous' id='${
                                  this.previous_id
                                }'><</button> Paths:</th>
                            </thead>
                            <tbody id='${this.paths_id}'>
                                <tr><td><b>Node</b></td><td><b>Distance</b></td><td><b>Forward</b></td></tr>
                                <tr id='${this.paths_id + "_" + this.id}'><td>${
      this.id
    }</td><td>0</td><td>${this.id}</td></tr>
                            </tbody>
                        </table>
                    </td></tr>
                    <tr><td>
                        <table style="width: 100%;">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><button id='${
                                      this.disable_id
                                    }' style='width: 100%'>Disable</button></td>
                                </tr>
                                <tr>
                                    <td><button style='width: 100%' id=${
                                      this.delete_id
  ...
  createForwardingTable() {
    ...
    for (const [target, step] of Object.entries(this.path)) {
      ...
      this.forwarding[target] += this.forwarding[target].includes(hop)
        ? ""
        : hop;
      this.forwarding[target] = this.forwarding[target]
        .split("")
        .reverse()
        .join("")[0];
    }
      ...
      while (Q.length != 0) {
        ...
        if (u !== null) {
          for (const [neighbor, weight] of Object.entries(
            ...
            if (
              neighbor == "length" ||
              Node.nodes[neighbor].disabled ||
              Node.disabled.includes([neighbor, Node.nodes[u].id]) ||
              Node.disabled.includes([Node.nodes[u].id, neighbor])
            ) {
              continue;
            }
